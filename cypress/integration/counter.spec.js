...
describe("카운터 앱 테스트", () => {
  ...
  it("- 버튼을 클릭 시 counter가 1감소한다.", () => {
    // 최소값이 0 이기 때문에, +1을 진행하고 -1 버튼을 클릭하는 방향으로 작성한다.
    cy.get(".increase-btn").click();
  ...
  it("+ 버튼을 눌렀을 때 counter가 10이 넘는 경우 더이상 증가하지 못한다. (Max 값이 10)", () => {
    // 11번 눌렀을 때 10 이면 성공
    for (let i = 0; i < 11; i++) {
      cy.get(".increase-btn").click();
    }
  ...
  it("- 버튼을 눌렀을 때 counter가 0보다 작아지는 경우 더이상 감소하지 못한다. (Min 값이 0)", () => {
    cy.get(".decrease-btn").click();
